ExAd_journal_fnc_addNote = {
	...
	if(count _text == 0)exitWith{
		["ErrorTitleAndText", ["Journal", "Nothing to save, write something first!!"]] call ExileClient_gui_toaster_addTemplateToast;
	};
...
ExAd_journal_fnc_removeNote = {
	...
	if(_index == -1)exitWith{
		["ErrorTitleAndText", ["Journal", "Select a note to remove!!"]] call ExileClient_gui_toaster_addTemplateToast;
	};
