	///////////////////////////////////////////////////////////////////////////////
	// Attachments
	///////////////////////////////////////////////////////////////////////////////
	class CUP_acc_Glock17_Flashlight 				{ quality = 1; price = 14; };
	class CUP_acc_ANPEQ_15 							{ quality = 1; price = 14; };
	class CUP_acc_ANPEQ_2 							{ quality = 1; price = 14; };
	class CUP_acc_Flashlight 						{ quality = 1; price = 14; };
	class CUP_acc_Flashlight_wdl 					{ quality = 1; price = 14; };
	class CUP_acc_Flashlight_desert 				{ quality = 1; price = 14; };
	class CUP_acc_XM8_light_module 					{ quality = 1; price = 14; };
	class CUP_acc_ANPEQ_2_camo 						{ quality = 1; price = 14; };
	class CUP_acc_ANPEQ_2_desert 					{ quality = 1; price = 14; };
	class CUP_acc_ANPEQ_2_grey 						{ quality = 1; price = 14; };
	class CUP_acc_CZ_M3X 							{ quality = 1; price = 14; };
	
	///////////////////////////////////////////////////////////////////////////////
	// Muzzle Attachments
	///////////////////////////////////////////////////////////////////////////////
	class CUP_muzzle_snds_AWM 						{ quality = 1; price = 25; };
	class CUP_muzzle_snds_SCAR_H 					{ quality = 1; price = 50; };
	class CUP_muzzle_mfsup_SCAR_H 					{ quality = 1; price = 50; };
	class CUP_muzzle_snds_XM8 						{ quality = 1; price = 50; };
	class CUP_muzzle_snds_G36_black 				{ quality = 1; price = 25; };
	class CUP_muzzle_snds_G36_desert 				{ quality = 1; price = 25; };
	class CUP_muzzle_snds_L85 						{ quality = 1; price = 25; };
	class CUP_muzzle_snds_M16_camo 					{ quality = 1; price = 25; };
	class CUP_muzzle_snds_M16 						{ quality = 1; price = 25; };
	class CUP_muzzle_snds_SCAR_L 					{ quality = 1; price = 50; };
	class CUP_muzzle_mfsup_SCAR_L 					{ quality = 1; price = 50; };
	class CUP_muzzle_PBS4 							{ quality = 1; price = 50; };
	class CUP_muzzle_PB6P9 							{ quality = 1; price = 50; };
	class CUP_muzzle_Bizon 							{ quality = 1; price = 25; };
	class CUP_muzzle_snds_M110 						{ quality = 1; price = 50; };
	class CUP_muzzle_snds_M14 						{ quality = 1; price = 50; };
	class CUP_muzzle_snds_M9 						{ quality = 1; price = 15; };
	class CUP_muzzle_snds_MicroUzi 					{ quality = 1; price = 25; };		
	
	///////////////////////////////////////////////////////////////////////////////
	// Optic Attachments
	///////////////////////////////////////////////////////////////////////////////
	class CUP_optic_PSO_1 							{ quality = 1; price = 60; };
	class CUP_optic_PSO_3 							{ quality = 1; price = 60; };
	class CUP_optic_Kobra 							{ quality = 1; price = 80; };
	class CUP_optic_NSPU 							{ quality = 1; price = 50; };
	class CUP_optic_PechenegScope 					{ quality = 1; price = 50; };
	class CUP_optic_HoloBlack 						{ quality = 1; price = 150; };
	class CUP_optic_HoloWdl 						{ quality = 1; price = 150; };
	class CUP_optic_HoloDesert 						{ quality = 1; price = 150; };
	class CUP_optic_Eotech533 						{ quality = 1; price = 150; };
	class CUP_optic_Eotech533Grey 					{ quality = 1; price = 150; };
	class CUP_optic_CompM4 							{ quality = 1; price = 50; };
	class CUP_optic_SUSAT 							{ quality = 1; price = 150; };
	class CUP_optic_RCO 							{ quality = 1; price = 150; };
	class CUP_optic_RCO_desert 						{ quality = 1; price = 150; };
	class CUP_optic_CompM2_Woodland2 				{ quality = 1; price = 150; };
	class CUP_optic_CompM2_Woodland 				{ quality = 1; price = 150; };
	class CUP_optic_CompM2_Black 					{ quality = 1; price = 150; };
	class CUP_optic_CompM2_Desert 					{ quality = 1; price = 150; };
	class CUP_optic_ACOG 							{ quality = 1; price = 200; };
	class CUP_optic_SB_3_12x50_PMII 				{ quality = 1; price = 150; };
	class CUP_optic_AN_PVS_4 						{ quality = 1; price = 150; };
	class CUP_optic_AN_PVS_10 						{ quality = 1; price = 150; };
	class CUP_optic_LeupoldMk4 						{ quality = 1; price = 400; };
	class CUP_optic_LeupoldM3LR 					{ quality = 1; price = 400; };
	class CUP_optic_LeupoldMk4_CQ_T 				{ quality = 1; price = 400; };
	class CUP_optic_LeupoldMk4_MRT_tan 				{ quality = 1; price = 400; };
	class CUP_optic_LeupoldMk4_10x40_LRT_Desert 	{ quality = 1; price = 400; };
	class CUP_optic_LeupoldMk4_10x40_LRT_Woodland 	{ quality = 1; price = 400; };
	class CUP_optic_Leupold_VX3 					{ quality = 1; price = 150; };
	class CUP_optic_ElcanM145 						{ quality = 1; price = 200; };
	class CUP_optic_ELCAN_SpecterDR 				{ quality = 1; price = 150; };
	class CUP_optic_SB_11_4x20_PM 					{ quality = 1; price = 600; };
	class CUP_optic_ZDDot 							{ quality = 1; price = 150; };
	class CUP_optic_MRad 							{ quality = 1; price = 200; };
	class CUP_optic_TrijiconRx01_black 				{ quality = 1; price = 200; };
	class CUP_optic_TrijiconRx01_desert 			{ quality = 1; price = 200; };	
	
	///////////////////////////////////////////////////////////////////////////////
	// Ammunition
	///////////////////////////////////////////////////////////////////////////////
	class CUP_100Rnd_556x45_BetaCMag    					{ quality = 2; price = 25; };
    class CUP_100Rnd_TE4_Green_Tracer_556x45_M249    		{ quality = 2; price = 50; };
    class CUP_100Rnd_TE4_LRT4_Red_Tracer_762x51_Belt_M    	{ quality = 2; price = 70; };
    class CUP_100Rnd_TE4_LRT4_762x54_PK_Tracer_Green_M    	{ quality = 2; price = 70; };
    class CUP_10Rnd_762x51_CZ750    						{ quality = 2; price = 2; };
    class CUP_10Rnd_762x54_SVD_M    						{ quality = 2; price = 100; };
    class CUP_10Rnd_9x19_Compact    						{ quality = 2; price = 18; };
    class CUP_10Rnd_9x39_SP5_VSS_M    						{ quality = 2; price = 18; };
    class CUP_10x_303_M    									{ quality = 2; price = 20; };
    class CUP_15Rnd_9x19_M9    								{ quality = 2; price = 15; };
    class CUP_17Rnd_9x19_glock17    						{ quality = 2; price = 15; };
    class CUP_18Rnd_9x19_Phantom    						{ quality = 2; price = 15; };
    class CUP_1Rnd_HE_GP25_M    							{ quality = 2; price = 40; };
    class CUP_1Rnd_HEDP_M203    							{ quality = 2; price = 40; };
    class CUP_1Rnd_SMOKE_GP25_M    							{ quality = 2; price = 6; };
    class CUP_1Rnd_Smoke_M203    							{ quality = 2; price = 6; };
    class CUP_1Rnd_SmokeGreen_M203    						{ quality = 2; price = 6; };
    class CUP_1Rnd_SmokeRed_M203    						{ quality = 2; price = 6; };
    class CUP_1Rnd_SmokeYellow_M203    						{ quality = 2; price = 6; };
    class CUP_200Rnd_TE4_Green_Tracer_556x45_L110A1    		{ quality = 2; price = 30; };
    class CUP_200Rnd_TE4_Green_Tracer_556x45_M249    		{ quality = 2; price = 35; };
    class CUP_20Rnd_762x51_DMR    							{ quality = 2; price = 25; };
    class CUP_20Rnd_762x51_FNFAL_M    						{ quality = 2; price = 25; };
    class CUP_20Rnd_762x51_B_SCAR    						{ quality = 2; price = 25; };
    class CUP_20Rnd_762x51_B_M110    						{ quality = 2; price = 45; };
    class CUP_20Rnd_762x51_CZ805B    						{ quality = 2; price = 4; };
    class CUP_30Rnd_9x19_EVO    							{ quality = 2; price = 15; };
    class CUP_20Rnd_B_AA12_Pellets    						{ quality = 2; price = 50; };
	class CUP_20Rnd_B_AA12_74Slug    						{ quality = 2; price = 50; };
    class CUP_30Rnd_545x39_AK_M    							{ quality = 2; price = 25; };
    class CUP_30Rnd_556x45_G36    							{ quality = 2; price = 20; };
    class CUP_30Rnd_556x45_Stanag    						{ quality = 2; price = 15; };
    class CUP_30Rnd_762x39_AK47_M    						{ quality = 2; price = 25; };
    class CUP_30Rnd_Sa58_M    								{ quality = 2; price = 15; };
    class CUP_30Rnd_9x19_MP5    							{ quality = 2; price = 25; };
    class CUP_5Rnd_127x108_KSVK_M    						{ quality = 2; price = 400; };
    class CUP_5Rnd_127x99_as50_M    						{ quality = 2; price = 500; };
    class CUP_5Rnd_762x51_M24    							{ quality = 2; price = 40; };
    class CUP_5Rnd_86x70_L115A1    							{ quality = 2; price = 400; };
    class CUP_5x_22_LR_17_HMR_M    							{ quality = 2; price = 25; };
    class CUP_64Rnd_9x19_Bizon_M    						{ quality = 2; price = 35; };
    class CUP_6Rnd_45ACP_M    								{ quality = 2; price = 35; };
    class CUP_75Rnd_TE4_LRT4_Green_Tracer_545x39_RPK_M    	{ quality = 2; price = 50; };
    class CUP_7Rnd_45ACP_1911    							{ quality = 2; price = 35; };
    class CUP_8Rnd_9x18_Makarov_M    						{ quality = 2; price = 25; };
    class CUP_8Rnd_9x18_MakarovSD_M    						{ quality = 2; price = 25; };
    class CUP_8Rnd_B_Beneli_74Slug    						{ quality = 2; price = 25; };
	class CUP_50Rnd_UK59_762x54R_Tracer						{ quality = 2; price = 50; };
    class CUP_8Rnd_B_Saiga12_74Slug_M    					{ quality = 2; price = 25; };
    class CUP_AT13_M    									{ quality = 2; price = 50; };
    class CUP_HandGrenade_L109A1_HE    						{ quality = 2; price = 125; };
    class CUP_Dragon_EP1_M    								{ quality = 2; price = 130; };
    class CUP_FlareGreen_M203    							{ quality = 2; price = 25; };
    class CUP_FlareRed_GP25_M    							{ quality = 2; price = 25; };
    class CUP_FlareWhite_GP25_M    							{ quality = 2; price = 25; };
    class CUP_FlareWhite_M203    							{ quality = 2; price = 25; };
    class CUP_HandGrenade_RGD5    							{ quality = 2; price = 35; };
    class CUP_HandGrenade_M67    							{ quality = 2; price = 35; };
	class CUP_6Rnd_HE_M203    								{ quality = 2; price = 125; };
	class CUP_1Rnd_HE_M203    								{ quality = 2; price = 25; };
    class CUP_Mine_M   										{ quality = 2; price = 12000; };
    class CUP_PipeBomb_M    								{ quality = 2; price = 25000; };	

	///////////////////////////////////////////////////////////////////////////////
	// Flares AND SMOKES
	///////////////////////////////////////////////////////////////////////////////
	class CUP_6Rnd_Smoke_M203						{ quality = 1; price = 60; };

	///////////////////////////////////////////////////////////////////////////////
	// Pistols
	///////////////////////////////////////////////////////////////////////////////
    class CUP_hgun_Colt1911 						{ quality = 1; price = 125; };
    class CUP_hgun_Compact 							{ quality = 1; price = 70; };
    class CUP_hgun_Makarov 							{ quality = 1; price = 70; };
    class CUP_hgun_MicroUzi 						{ quality = 1; price = 125; };
    class CUP_hgun_TaurusTracker455 				{ quality = 1; price = 400; };
    class CUP_hgun_TaurusTracker455_gold 			{ quality = 1; price = 600; };
    class CUP_hgun_M9 								{ quality = 1; price = 125; };
    class CUP_hgun_SA61 							{ quality = 1; price = 125; };
    class CUP_hgun_Duty 							{ quality = 1; price = 125; };
    class CUP_hgun_Phantom 							{ quality = 1; price = 125; };
    class CUP_hgun_PB6P9 							{ quality = 1; price = 125; };
    class CUP_hgun_Glock17 							{ quality = 1; price = 125; };	

	///////////////////////////////////////////////////////////////////////////////
	// Sub Machine Guns
	///////////////////////////////////////////////////////////////////////////////
    class CUP_smg_MP5SD6 							{ quality = 1; price = 300; };
    class CUP_smg_MP5A5 							{ quality = 1; price = 350; };
    class CUP_smg_EVO 								{ quality = 1; price = 300; };
    class CUP_smg_bizon 							{ quality = 1; price = 325; };
	
	///////////////////////////////////////////////////////////////////////////////
	// Light Machine Guns
	///////////////////////////////////////////////////////////////////////////////
    class CUP_lmg_M249_para 						{ quality = 1; price = 2500; };
    class CUP_lmg_Mk48_des 							{ quality = 1; price = 2500; };
    class CUP_lmg_Mk48_wdl 							{ quality = 1; price = 2500; };
    class CUP_lmg_L110A1 							{ quality = 1; price = 2500; };
    class CUP_lmg_Pecheneg 							{ quality = 1; price = 2500; };
    class CUP_lmg_UK59 								{ quality = 1; price = 2500; };
    class CUP_lmg_L7A2 								{ quality = 1; price = 2500; };
    class CUP_lmg_M60A4 							{ quality = 1; price = 2500; };
    class CUP_lmg_M240 								{ quality = 1; price = 2500; };
    class CUP_lmg_M249 								{ quality = 1; price = 2500; };
    class CUP_lmg_PKM 								{ quality = 1; price = 2500; };	
	
	///////////////////////////////////////////////////////////////////////////////
	// Explosive and Shotgun
	///////////////////////////////////////////////////////////////////////////////
	class CUP_sgun_AA12 							{ quality = 1; price = 3500; };
    class CUP_sgun_M1014 							{ quality = 1; price = 750; };
    class CUP_sgun_Saiga12K 						{ quality = 1; price = 750; };

	///////////////////////////////////////////////////////////////////////////////
	// Assault Rifles
	///////////////////////////////////////////////////////////////////////////////	
    class CUP_arifle_Mk16_CQC_FG 					{ quality = 1; price = 850; };
    class CUP_arifle_Mk16_CQC_SFG 					{ quality = 1; price = 850; };
    class CUP_arifle_Mk16_CQC_EGLM 					{ quality = 1; price = 850; };
    class CUP_arifle_Mk16_STD 						{ quality = 1; price = 850; };
    class CUP_arifle_Mk16_STD_FG 					{ quality = 1; price = 850; };
    class CUP_arifle_Mk16_STD_SFG 					{ quality = 1; price = 850; };
    class CUP_arifle_Mk16_STD_EGLM 					{ quality = 1; price = 1000; };
    class CUP_arifle_Mk16_SV 						{ quality = 1; price = 850; };
    class CUP_arifle_Mk17_CQC 						{ quality = 1; price = 1200; };
    class CUP_arifle_Mk17_CQC_FG 					{ quality = 1; price = 1200; };
    class CUP_arifle_Mk17_CQC_SFG 					{ quality = 1; price = 1200; };
    class CUP_arifle_Mk17_CQC_EGLM 					{ quality = 1; price = 1500; };
    class CUP_arifle_Mk17_STD 						{ quality = 1; price = 1200; };
    class CUP_arifle_Mk17_STD_FG 					{ quality = 1; price = 1200; };
    class CUP_arifle_Mk17_STD_SFG 					{ quality = 1; price = 1200; };
    class CUP_arifle_Mk17_STD_EGLM 					{ quality = 1; price = 1500; };
    class CUP_arifle_Mk20 							{ quality = 1; price = 1200; };
    class CUP_arifle_XM8_Compact_Rail 				{ quality = 1; price = 850; };
    class CUP_arifle_XM8_Railed 					{ quality = 1; price = 850; };
    class CUP_arifle_M16A4_Base 					{ quality = 1; price = 850; };
    class CUP_arifle_M4A1_BUIS_GL 					{ quality = 1; price = 850; };
    class CUP_arifle_M4A1_BUIS_camo_GL 				{ quality = 1; price = 850; };
    class CUP_arifle_M4A1_BUIS_desert_GL 			{ quality = 1; price = 850; };
    class CUP_arifle_M4A1_black 					{ quality = 1; price = 850; };
    class CUP_arifle_M4A1_desert 					{ quality = 1; price = 850; };
    class CUP_arifle_L85A2 							{ quality = 1; price = 850; };
    class CUP_arifle_L85A2_GL 						{ quality = 1; price = 850; };
    class CUP_arifle_L86A2 							{ quality = 1; price = 850; };
    class CUP_arifle_FNFAL_railed 					{ quality = 1; price = 1200; };
    class CUP_arifle_AK107_GL 						{ quality = 1; price = 850; };
    class CUP_arifle_Sa58P_des 						{ quality = 1; price = 1200; };
    class CUP_arifle_Sa58V_camo 					{ quality = 1; price = 1200; };
    class CUP_arifle_Sa58RIS1 						{ quality = 1; price = 1200; };
    class CUP_arifle_Sa58RIS1_des 					{ quality = 1; price = 1200; };
    class CUP_arifle_Sa58RIS2 						{ quality = 1; price = 1200; };
    class CUP_arifle_Sa58RIS2_camo 					{ quality = 1; price = 1200; };
    class CUP_arifle_CZ805_A1 						{ quality = 1; price = 850; };
    class CUP_arifle_CZ805_GL 						{ quality = 1; price = 850; };
    class CUP_arifle_CZ805_B_GL 					{ quality = 1; price = 850; };
    class CUP_arifle_CZ805_B 						{ quality = 1; price = 850; };
    class CUP_arifle_XM8_Carbine 					{ quality = 1; price = 850; };
    class CUP_arifle_XM8_Carbine_FG 				{ quality = 1; price = 850; };
    class CUP_arifle_XM8_Carbine_GL 				{ quality = 1; price = 850; };
    class CUP_arifle_XM8_Compact 					{ quality = 1; price = 850; };
    class CUP_arifle_xm8_SAW 						{ quality = 1; price = 1200; };
    class CUP_arifle_xm8_sharpshooter 				{ quality = 1; price = 850; };
    class CUP_arifle_Mk16_CQC 						{ quality = 1; price = 850; };
    class CUP_arifle_Sa58P 							{ quality = 1; price = 1200; };
    class CUP_arifle_Sa58V 							{ quality = 1; price = 1200; };
    class CUP_arifle_M16A2 							{ quality = 1; price = 850; };
    class CUP_arifle_M16A2_GL 						{ quality = 1; price = 850; };
    class CUP_arifle_M16A4_GL 						{ quality = 1; price = 850; };
    class CUP_arifle_M4A1 							{ quality = 1; price = 850; };
    class CUP_arifle_M4A1_camo 						{ quality = 1; price = 850; };
    class CUP_arifle_FNFAL 							{ quality = 1; price = 1200; };
    class CUP_arifle_G36A 							{ quality = 1; price = 850; };
    class CUP_arifle_G36A_camo 						{ quality = 1; price = 850; };
    class CUP_arifle_G36K 							{ quality = 1; price = 850; };
    class CUP_arifle_G36K_camo 						{ quality = 1; price = 850; };
    class CUP_arifle_G36C 							{ quality = 1; price = 850; };
    class CUP_arifle_G36C_camo 						{ quality = 1; price = 850; };
    class CUP_arifle_MG36 							{ quality = 1; price = 850; };
    class CUP_arifle_MG36_camo 						{ quality = 1; price = 850; };
    class CUP_arifle_AK74 							{ quality = 1; price = 850; };
    class CUP_arifle_AK107 							{ quality = 1; price = 850; };
    class CUP_arifle_AKS74 							{ quality = 1; price = 850; };
    class CUP_arifle_AKS74U 						{ quality = 1; price = 850; };
    class CUP_arifle_AK74_GL 						{ quality = 1; price = 850; };
    class CUP_arifle_AKM 							{ quality = 1; price = 1200; };
    class CUP_arifle_AKS 							{ quality = 1; price = 1200; };
    class CUP_arifle_AKS_Gold 						{ quality = 1; price = 2900; };
    class CUP_arifle_RPK74 							{ quality = 1; price = 2500; };
    class CUP_arifle_AK74_GL_kobra 					{ quality = 1; price = 800; };
    class CUP_arifle_CZ805_A2 						{ quality = 1; price = 1200; };
	
	///////////////////////////////////////////////////////////////////////////////
	// Sniper Rifles
	///////////////////////////////////////////////////////////////////////////////	
    class CUP_srifle_SVD_wdl_ghillie 				{ quality = 1; price = 2600; };
    class CUP_srifle_SVD_des_ghillie_pso 			{ quality = 1; price = 2600; };
    class CUP_srifle_DMR 							{ quality = 1; price = 1200; };
    class CUP_srifle_ksvk 							{ quality = 1; price = 3500; };
    class CUP_srifle_M110 							{ quality = 1; price = 1500; };
    class CUP_srifle_M14 							{ quality = 1; price = 1200; };
    class CUP_srifle_M24_des 						{ quality = 1; price = 1200; };
    class CUP_srifle_M24_wdl 						{ quality = 1; price = 1200; };
    class CUP_srifle_M24_ghillie 					{ quality = 1; price = 1200; };
    class CUP_srifle_SVD 							{ quality = 1; price = 1200; };
    class CUP_srifle_SVD_des 						{ quality = 1; price = 1200; };
    class CUP_srifle_VSSVintorez 					{ quality = 1; price = 1100; };
    class CUP_srifle_CZ750 							{ quality = 1; price = 500; };
    class CUP_srifle_Mk12SPR 						{ quality = 1; price = 1700; };
    class CUP_srifle_AS50 							{ quality = 1; price = 11500; };
    class CUP_srifle_AWM_des 						{ quality = 1; price = 2100; };
    class CUP_srifle_AWM_wdl 						{ quality = 1; price = 2100; };
    class CUP_srifle_CZ550 							{ quality = 1; price = 1200; };
    class CUP_srifle_LeeEnfield 					{ quality = 1; price = 1200; };
    class CUP_srifle_M40A3 							{ quality = 1; price = 1200; };	
	
	///////////////////////////////////////////////////////////////////////////////
	// Launchers
	///////////////////////////////////////////////////////////////////////////////
    class CUP_glaunch_M32 							{ quality = 1; price = 15000; };
    class CUP_glaunch_M79 							{ quality = 1; price = 15000; };
    class CUP_glaunch_Mk13                          { quality = 1; price = 15000; };
    class CUP_launch_RPG7V      					{ quality = 1; price = 25000; };
	
	///////////////////////////////////////////////////////////////////////////////
	// Launcher Ammo
	///////////////////////////////////////////////////////////////////////////////
    class CUP_RPG18_M    							{ quality = 2; price = 4500; };
    class CUP_PG7VL_M    							{ quality = 2; price = 4500; };
    class CUP_PG7VR_M    							{ quality = 2; price = 4500; };	
    class CUP_M136_M                                { quality = 2; price = 1500; };