	///////////////////////////////////////////////////////////////////////////////
	// RHS Custom Stuff
	///////////////////////////////////////////////////////////////////////////////
	
	//UNIFORMS
	class rhs_uniform_cu_ocp						{ quality = 3; price = 50; };
	class rhs_uniform_cu_ucp						{ quality = 3; price = 50; };
	class rhs_uniform_cu_ocp_101st                  { quality = 3; price = 50; };
	class rhs_uniform_emr_patchless					{ quality = 2; price = 50; };
	class rhs_uniform_flora_patchless				{ quality = 2; price = 50; };
	class rhs_uniform_flora_patchless_alt			{ quality = 2; price = 50; };
	class rhs_uniform_mvd_izlom						{ quality = 2; price = 50; };
	class rhs_uniform_m88_patchless					{ quality = 2; price = 50; };
	class rhs_uniform_mflora_patchless				{ quality = 2; price = 50; };
	class rhs_uniform_vdv_mflora                    { quality = 2; price = 55; };
	class rhs_uniform_FROG01_m81					{ quality = 3; price = 65; };
	class rhs_uniform_FROG01_d						{ quality = 3; price = 65; };
	class rhs_uniform_FROG01_wd						{ quality = 3; price = 65; };
	class rhs_chdkz_uniform_1                       { quality = 2; price = 60; };
	class rhs_chdkz_uniform_2                       { quality = 2; price = 60; };
	class rhs_chdkz_uniform_3                       { quality = 2; price = 60; };
	class rhs_chdkz_uniform_4                       { quality = 2; price = 60; };
	class rhs_chdkz_uniform_5                       { quality = 2; price = 60; };

	//VESTS
	class rhs_6sh92								    { quality = 2; price = 50; };
	class rhs_6sh92_radio							{ quality = 2; price = 50; };
	class rhs_6sh92_vog								{ quality = 2; price = 50; };
	class rhs_6sh92_vog_headset						{ quality = 2; price = 50; };
	class rhs_6sh92_headset							{ quality = 2; price = 50; };
	class rhs_6sh92_digi							{ quality = 2; price = 50; };
	class rhs_6sh92_digi_radio					    { quality = 2; price = 50; };
	class rhs_6sh92_digi_vog						{ quality = 2; price = 50; };
	class rhs_6sh92_digi_vog_headset				{ quality = 2; price = 50; };
	class rhs_6sh92_digi_headset					{ quality = 2; price = 50; };
	class rhs_vydra_3m                              { quality = 1; price = 30; };
    class rhs_6b13                            	  { quality = 2; price = 700; };
    class rhs_6b13_EMR                    	      { quality = 2; price = 700; };
    class rhsusf_iotv_ocp                   	    { quality = 2; price = 750; };
    class rhsusf_iotv_ocp_Rifleman         	     { quality = 2; price = 1300; };
    class rhsusf_iotv_ocp_Squadleader           { quality = 2; price = 1300; };
	class rhsusf_spc_rifleman                   { quality = 2; price = 6000; };
	
	//HELMETS
	class rhs_6b26                                  { quality = 2; price = 60; };
    class rhs_6b26_ess                              { quality = 2; price = 60; };
    class rhs_6b26_bala_green                       { quality = 2; price = 60; };
    class rhs_6b27m                                 { quality = 2; price = 60; };
	class rhs_6b27m_ml                          { quality = 2; price = 60; };
	class rhs_6b27m_green_ess                   { quality = 2; price = 60; };
	class rhs_6b28_flora 	                    { quality = 2; price = 60; };
	class rhs_6b28_ess 	                        { quality = 2; price = 60; };
	class rhs_6b28_flora_ess                    { quality = 2; price = 60; };
	class rhs_6b28_green_ess_bala               { quality = 2; price = 60; };
	class rhs_tsh4 	                            { quality = 2; price = 10; };
	class rhs_tsh4_ess 	                        { quality = 2; price = 10; };
	class rhs_tsh4_bala                         { quality = 2; price = 10; };
	class rhsusf_lwh_helmet_marpatwd_ess         { quality = 2; price = 60; };
	class rhsusf_lwh_helmet_marpatwd             { quality = 2; price = 60; };
	class rhsusf_mich_helmet_marpatwd                { quality = 2; price = 60; };
	class rhsusf_mich_helmet_marpatwd_headset        { quality = 2; price = 60; };
	class rhsusf_mich_helmet_marpatwd_norotos        { quality = 2; price = 60; };
	class rhsusf_mich_bare                           { quality = 2; price = 60; };
	class rhsusf_mich_bare_alt                       { quality = 2; price = 60; };
	class rhsusf_mich_bare_tan                       { quality = 2; price = 60; };
	class rhs_ssh68                                  { quality = 2; price = 30; };

	//Pointer Attachments
	class rhsusf_acc_anpeq15side					{ quality = 2; price = 50; };
	class rhsusf_acc_anpeq15						{ quality = 2; price = 50; };
	class rhsusf_acc_anpeq15A						{ quality = 1; price = 50; };
	class rhsusf_acc_anpeq15_light					{ quality = 2; price = 50; };
	
	//Bipod
	class rhsusf_acc_harris_bipod					{ quality = 3; price = 30; };
	
	//Muzzle
	//US	
	class rhsusf_acc_SF3P556						{ quality = 2; price = 40; };
	class rhsusf_acc_SFMB556						{ quality = 2; price = 40; };
	class rhsusf_acc_nt4_black						{ quality = 3; price = 80; };
	class rhsusf_acc_nt4_tan						{ quality = 3; price = 80; };
	class rhsusf_acc_rotex5_grey					{ quality = 3; price = 80; };
	class rhsusf_acc_rotex5_tan						{ quality = 3; price = 80; };
	class rhsusf_acc_SR25S							{ quality = 3; price = 90; };
	class rhsusf_acc_M2010S							{ quality = 3; price = 100; };
	//Russian
	class rhs_acc_ak5								{ quality = 1; price = 40; };
	class rhs_acc_dtk								{ quality = 1; price = 40; };
	class rhs_acc_dtk1								{ quality = 1; price = 40; };
	class rhs_acc_dtk3								{ quality = 1; price = 40; };
	class rhs_acc_dtk4screws						{ quality = 2; price = 80; };
	class rhs_acc_dtk4long							{ quality = 2; price = 80; };
	class rhs_acc_dtk4short							{ quality = 2; price = 80; };
	class rhs_acc_tgpa								{ quality = 2; price = 80; };
	class rhs_acc_dtk1l								{ quality = 1; price = 40; };
	class rhs_acc_pbs1								{ quality = 2; price = 90; };	
	
	//Optics
	//US
	class rhsusf_acc_eotech_552						{ quality = 2; price = 75; };
	class rhsusf_acc_compm4							{ quality = 2; price = 50; };
	class rhsusf_acc_ACOG2_USMC						{ quality = 2; price = 300; };
	class rhsusf_acc_ACOG3_USMC						{ quality = 2; price = 300; };
	class rhsusf_acc_ACOG_USMC						{ quality = 2; price = 300; };
	class rhsusf_acc_ACOG							{ quality = 2; price = 300; };
	class rhsusf_acc_ACOG_pip						{ quality = 2; price = 300; };
	class rhsusf_acc_ACOG2							{ quality = 2; price = 300; };
	class rhsusf_acc_ACOG3							{ quality = 2; price = 300; };
	class rhsusf_acc_ELCAN							{ quality = 3; price = 500; };
	class rhsusf_acc_ELCAN_pip						{ quality = 3; price = 500; };
	class rhsusf_acc_LEUPOLDMK4 					{ quality = 3; price = 850; };
	class rhsusf_acc_LEUPOLDMK4_2 					{ quality = 3; price = 850; };
	//Russian
	class rhs_acc_1pn93_2                           { quality = 1; price = 50; };
	class rhs_acc_1p29								{ quality = 2; price = 300; };
	class rhs_acc_1p63								{ quality = 2; price = 75; };
	class rhs_acc_1p78								{ quality = 1; price = 250; };
	class rhs_acc_1pn93_1							{ quality = 1; price = 300; };
	class rhs_acc_ekp1								{ quality = 1; price = 50; };
	class rhs_acc_pgo7v								{ quality = 2; price = 300; };
	class rhs_acc_pkas								{ quality = 1; price = 50; };
	class rhs_acc_pso1m2							{ quality = 2; price = 300; };
	class rhs_acc_pso1m21                           { quality = 1; price = 70; };
	
	//AMMO
	class rhsusf_mag_7x45acp_MHP                    { quality = 1; price = 6; };
	class rhs_mag_9x18_12_57N181S                   { quality = 1; price = 6; };
	class rhs_mag_9x19_17                           { quality = 1; price = 6; };
	//US
	class rhs_mag_30Rnd_556x45_Mk318_Stanag						{ quality = 2; price = 35; };
	class rhs_mag_30Rnd_556x45_Mk262_Stanag						{ quality = 2; price = 40; };
	class rhs_mag_30Rnd_556x45_M855A1_Stanag					{ quality = 2; price = 30; };
	class rhs_mag_30Rnd_556x45_M855A1_Stanag_No_Tracer			{ quality = 2; price = 30; };
	class rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Red			{ quality = 2; price = 30; };
	class rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Green		{ quality = 2; price = 30; };
	class rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Yellow		{ quality = 2; price = 30; };
	class rhs_200rnd_556x45_M_SAW								{ quality = 3; price = 120; };
	class rhs_200rnd_556x45_B_SAW								{ quality = 3; price = 120; };
	class rhs_200rnd_556x45_T_SAW								{ quality = 3; price = 120; };
	class rhsusf_50Rnd_762x51									{ quality = 3; price = 50; };
	class rhsusf_50Rnd_762x51_m993								{ quality = 3; price = 50; };
	class rhsusf_50Rnd_762x51_m80a1epr							{ quality = 3; price = 95; };
	class rhsusf_100Rnd_762x51									{ quality = 3; price = 95; };
	class rhsusf_100Rnd_762x51_m993								{ quality = 3; price = 95; };
	class rhsusf_100Rnd_762x51_m80a1epr							{ quality = 3; price = 95; };
	class rhsusf_5Rnd_00Buck									{ quality = 3; price = 40; };
	class rhsusf_5Rnd_Slug										{ quality = 3; price = 30; };
	class rhsusf_8Rnd_00Buck									{ quality = 3; price = 20; };
	class rhsusf_8Rnd_Slug										{ quality = 3; price = 50; };
	class rhsusf_20Rnd_762x51_m993_Mag							{ quality = 3; price = 45; };
	class rhsusf_5Rnd_300winmag_xm2010							{ quality = 3; price = 55; };
	class rhsusf_8Rnd_HE  										{ quality = 1; price = 20; };
    class rhsusf_8Rnd_FRAG  									{ quality = 1; price = 20; };
	class rhsusf_100Rnd_762x51_m61_ap 							{ quality = 1; price = 100; };							
	//Russian
	class rhs_30Rnd_762x39mm									{ quality = 1; price = 20; };
	class rhs_30Rnd_762x39mm_tracer								{ quality = 1; price = 20; };
	class rhs_30Rnd_762x39mm_89									{ quality = 1; price = 20; };
	class rhs_30Rnd_545x39_AK									{ quality = 1; price = 20; };
	class rhs_30Rnd_545x39_AK_no_tracers						{ quality = 1; price = 20; };
	class rhs_30Rnd_545x39_7N10_AK								{ quality = 1; price = 20; };
	class rhs_30Rnd_545x39_7N22_AK								{ quality = 1; price = 20; };
	class rhs_30Rnd_545x39_AK_green								{ quality = 1; price = 20; };
	class rhs_45Rnd_545X39_AK									{ quality = 1; price = 40; };
	class rhs_45Rnd_545X39_7N10_AK								{ quality = 1; price = 40; };
	class rhs_45Rnd_545X39_7N22_AK								{ quality = 1; price = 40; };
	class rhs_45Rnd_545X39_AK_Green								{ quality = 1; price = 40; };
	class rhs_100Rnd_762x54mmR									{ quality = 2; price = 95; };
	class rhs_100Rnd_762x54mmR_green							{ quality = 2; price = 95; };
	class rhs_10Rnd_762x54mmR_7N1								{ quality = 2; price = 50; };	
	
	//WEAPONS
	//PISTOL
	class rhs_weap_pya								{ quality = 3; price = 100; };
	class rhs_weap_makarov_pmm						{ quality = 3; price = 100; };
	class rhsusf_weap_m1911a1						{ quality = 3; price = 100; };
	class rhsusf_weap_glock17g4                     { quality = 1; price = 60; };
	class rhsusf_weap_m9                            { quality = 1; price = 60; };
	
	//LMG
	//US
	class rhs_weap_m249_pip_L						{ quality = 3; price = 850; };
	class rhs_weap_m249_pip_L_para					{ quality = 3; price = 850; };
	class rhs_weap_m249_pip_L_vfg					{ quality = 3; price = 850; };
	class rhs_weap_m249_pip_S						{ quality = 3; price = 850; };
	class rhs_weap_m249_pip_S_para					{ quality = 3; price = 850; };
	class rhs_weap_m249_pip_S_vfg					{ quality = 3; price = 850; };
	class rhs_weap_m240B							{ quality = 3; price = 800; };
	class rhs_weap_m240B_CAP						{ quality = 3; price = 800; };
	class rhs_weap_m240G							{ quality = 3; price = 800; };
	//Russian
	class rhs_weap_pkm								{ quality = 2; price = 250; };
	class rhs_weap_pkp								{ quality = 2; price = 300; };	

	//RPG's
	class rhs_rpg7_PG7VL_mag						{ quality = 2; price = 3500; };
	class rhs_rpg7_PG7VR_mag						{ quality = 2; price = 3500; };
	class rhs_rpg7_TBG7V_mag						{ quality = 2; price = 3500; };
	class rhs_rpg7_OG7V_mag							{ quality = 2; price = 3500; };
	//ASSAULT RIFLES
	//US
	class rhs_weap_M590_5RD							{ quality = 3; price = 350; };
	class rhs_weap_M590_8RD							{ quality = 3; price = 350; };
	class rhs_weap_m16a4							{ quality = 1; price = 350; };
	class rhs_weap_m16a4_carryhandle				{ quality = 1; price = 350; };
	class rhs_weap_m16a4_carryhandle_grip			{ quality = 2; price = 400; };
	class rhs_weap_m16a4_carryhandle_grip_pmag		{ quality = 2; price = 400; };
	class rhs_weap_m16a4_carryhandle_M203			{ quality = 2; price = 400; };
	class rhs_weap_m16a4_carryhandle_pmag			{ quality = 2; price = 400; };
	class rhs_weap_m16a4_grip						{ quality = 1; price = 350; };
	class rhs_weap_m4								{ quality = 1; price = 350; };
	class rhs_weap_m4_grip2							{ quality = 2; price = 400; };
	class rhs_weap_m4_carryhandle					{ quality = 1; price = 350; };
	class rhs_weap_m4_carryhandle_pmag				{ quality = 1; price = 350; };
	class rhs_weap_m4_grip							{ quality = 2; price = 400; };
	class rhs_weap_m4_m203							{ quality = 3; price = 450; };
	class rhs_weap_m4_m320							{ quality = 3; price = 450; };
	class rhs_weap_m4a1_carryhandle					{ quality = 1; price = 350; };
	class rhs_weap_m4a1_carryhandle_grip2			{ quality = 2; price = 400; };
	class rhs_weap_m4a1_carryhandle_pmag			{ quality = 1; price = 350; };
	class rhs_weap_m4a1_carryhandle_m203			{ quality = 3; price = 450; };
	class rhs_weap_m4a1								{ quality = 1; price = 350; };
	class rhs_weap_m4a1_grip2						{ quality = 2; price = 400; };
	class rhs_weap_m4a1_grip						{ quality = 2; price = 400; };
	class rhs_weap_m4a1_m203						{ quality = 3; price = 450; };
	class rhs_weap_m4a1_m320						{ quality = 3; price = 450; };
	class rhs_weap_m4a1_blockII						{ quality = 3; price = 500; };
	class rhs_weap_m4a1_blockII_KAC					{ quality = 3; price = 500; };
	class rhs_weap_m4a1_blockII_grip2				{ quality = 3; price = 500; };
	class rhs_weap_m4a1_blockII_grip2_KAC			{ quality = 3; price = 500; };
	class rhs_weap_m4a1_blockII_M203				{ quality = 3; price = 500; };			
	class rhs_weap_mk18								{ quality = 3; price = 500; };
	class rhs_weap_mk18_KAC							{ quality = 3; price = 500; };
	class rhs_weap_mk18_grip2						{ quality = 3; price = 500; };
	class rhs_weap_mk18_grip2_KAC					{ quality = 3; price = 500; };
	class rhs_weap_mk18_m320						{ quality = 3; price = 500; };

	//Russian
	class rhs_weap_ak103							{ quality = 1; price = 150; };
	class rhs_weap_ak103_npz						{ quality = 1; price = 150; };
	class rhs_weap_ak103_1							{ quality = 1; price = 150; };
	class rhs_weap_ak74m							{ quality = 1; price = 150; };
	class rhs_weap_ak74m_2mag						{ quality = 1; price = 150; };
	class rhs_weap_ak74m_2mag_camo					{ quality = 2; price = 200; };
	class rhs_weap_ak74m_2mag_npz					{ quality = 1; price = 200; };
	class rhs_weap_ak74m_camo						{ quality = 2; price = 200; };
	class rhs_weap_ak74m_desert						{ quality = 2; price = 200; };
	class rhs_weap_ak74m_desert_npz					{ quality = 2; price = 250; };
	class rhs_weap_ak74m_desert_folded				{ quality = 1; price = 150; };
	class rhs_weap_ak74m_plummag_folded				{ quality = 1; price = 150; };
	class rhs_weap_ak74m_folded						{ quality = 1; price = 150; };
	class rhs_weap_ak74m_camo_folded				{ quality = 1; price = 150; };
	class rhs_weap_ak74m_gp25						{ quality = 2; price = 300; };
	class rhs_weap_ak74m_gp25_npz					{ quality = 2; price = 300; };
	class rhs_weap_ak74m_npz						{ quality = 1; price = 200; };
	class rhs_weap_ak74m_plummag					{ quality = 2; price = 250; };
	class rhs_weap_ak74m_plummag_npz				{ quality = 2; price = 300; };
	class rhs_weap_akm								{ quality = 1; price = 150; };
	class rhs_weap_akm_gp25							{ quality = 1; price = 250; };
	class rhs_weap_akms								{ quality = 1; price = 150; };
	class rhs_weap_akms_gp25						{ quality = 1; price = 200; };	
	class rhs_weap_asval                            { quality = 1; price = 350; };
	class rhs_weap_asval_npz                        { quality = 1; price = 400; };
	class rhs_weap_ak104                            { quality = 1; price = 300; };
	class rhs_weap_ak104_npz                        { quality = 1; price = 350; };
	class rhs_weap_ak105                            { quality = 1; price = 200; };
	class rhs_weap_ak105_npz                        { quality = 1; price = 250; };
	
	//SNIPERS
	//US
	//class rhs_weap_M107								{ quality = 3; price = 1000; };
	class rhs_weap_sr25								{ quality = 3; price = 1000; };
	class rhs_weap_sr25_ec							{ quality = 3; price = 1000; };
	class rhs_weap_m14ebrri							{ quality = 3; price = 1000; };
	class rhs_weap_XM2010							{ quality = 3; price = 1250; };
	class rhs_weap_XM2010_wd						{ quality = 3; price = 1250; };
	class rhs_weap_XM2010_d							{ quality = 3; price = 1250; };
	class rhs_weap_XM2010_sa						{ quality = 3; price = 1250; };
	class rhs_weap_m110                             { quality = 1; price = 850; };
	//Russian
	class rhs_weap_svd								{ quality = 3; price = 1000; };
	class rhs_weap_svdp_wd							{ quality = 3; price = 1000; };
	class rhs_weap_svdp_wd_npz						{ quality = 3; price = 1000; };
	class rhs_weap_svdp_npz							{ quality = 3; price = 1000; };
	class rhs_weap_svds								{ quality = 3; price = 1000; };
	class rhs_weap_svds_npz							{ quality = 3; price = 1000; };
	 
	// RHS Mines
	class rhs_mine_pmn2_mag 						{ quality = 1; price = 1500; };
	class rhs_mine_tm62m_mag 						{ quality = 1; price = 1500; };
	class rhsusf_m112_mag 							{ quality = 1; price = 2000; };
	class rhsusf_m112x4_mag							{ quality = 1; price = 6000; };
	class rhs_mine_m19_mag 							{ quality = 1; price = 1500; };
	class rhs_weap_rpg7 							{ quality = 1; price = 25000; };
	

	///////////////////////////////////////////////////////////////////////////////
	// Explosives
	///////////////////////////////////////////////////////////////////////////////

	class rhs_mag_m4009								{ quality = 3; price = 80; };
	class rhs_mag_m576								{ quality = 3; price = 80; };
	class rhs_VOG25									{ quality = 3; price = 80; };
	class rhs_VG40SZ								{ quality = 3; price = 80; };
	class RHS_VOG25 				                { quality = 3; price = 30; };
	class rhs_VG40TB                                { quality = 3; price = 25; };
	class rhs_VG40MD_Green                          { quality = 3; price = 20; };
	class rhs_VG40MD_Red                            { quality = 3; price = 20; };
	class rhs_VG40MD_White                          { quality = 3; price = 20; };
	class rhs_VG40OP_green                          { quality = 3; price = 20; };
	class rhs_VG40OP_red                            { quality = 3; price = 20; };
	class rhs_VG40OP_white                          { quality = 3; price = 20; };

	///////////////////////////////////////////////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////////
	// VEHICLES
	///////////////////////////////////////////////////////////////////////////////
	class     rhsusf_m1025_d                            { quality = 1; price = 2000; };
	class     rhsusf_m1025_w                            { quality = 1; price = 2000; };
	class     rhsusf_m1025_w_s                          { quality = 1; price = 5000; };
	class     rhsusf_m1025_d_s                          { quality = 1; price = 5000; };
	class     rhsusf_m998_w_2dr                         { quality = 1; price = 5000; };
	class     rhsusf_m998_d_s_2dr                       { quality = 1; price = 7000; };
	class     rhs_tigr_vmf                              { quality = 1; price = 4000; };
	class     rhs_tigr_3camo_vmf                        { quality = 1; price = 4000; };
	class     rhs_tigr_ffv_vmf                          { quality = 1; price = 4000; };
	class     rhs_tigr_ffv_3camo_vdv                    { quality = 1; price = 4000; };
	class     rhs_tigr_ffv_3camo_vv                     { quality = 1; price = 4000; };
	class     rhs_tigr_m_msv                            { quality = 1; price = 10000; };
	class     rhs_tigr_m_3camo_vdv                      { quality = 1; price = 10000; };
	class     rhsusf_m998_w_2dr_fulltop                 { quality = 1; price = 5000; };
	class     rhsusf_m998_d_2dr_fulltop                 { quality = 1; price = 5000; };
	class     RHS_UAZ_MSV_01                            { quality = 1; price = 800; };
	class     rhs_uaz_open_MSV_01                       { quality = 1; price = 650; };
	class     rhs_uaz_dshkm_chdkz                       { quality = 1; price = 3500; };
	class     RHS_Ural_Flat_MSV_01                      { quality = 1; price = 4000; };
	class     RHS_Ural_MSV_01                           { quality = 1; price = 4000; };
	class     RHS_Ural_Open_Civ_03                      { quality = 1; price = 3500; };
	class     rhsusf_m1025_w_m2                         { quality = 1; price = 15000; };
	class     rhsusf_m1025_d_m2                         { quality = 1; price = 15000; };
	class     rhsusf_M1078A1P2_B_M2_wd_fmtv_usarmy      { quality = 1; price = 50000; };
	class     rhsusf_M1078A1P2_B_M2_wd_flatbed_fmtv_usarmy  { quality = 1; price = 50000; };
	class     rhsusf_M1078A1P2_B_M2_d_fmtv_usarmy  { quality = 1; price = 50000; };
	class     rhsusf_M1078A1P2_B_M2_d_flatbed_fmtv_usarmy  { quality = 1; price = 50000; };
	class     B_Boat_Armed_01_minigun_F                 { quality = 1; price = 9000; };
	class 	  rhs_btr60_chdkz 							{ quality = 1; price = 75000; };
	
	///////////////////////////////////////////////////////////////////////////////
	// AIR 
	//////////////////////////////////////////////////////////////////////////////
	class     RHS_UH60M_MEV                             { quality = 1; price = 16000; };
	class     RHS_UH1Y_UNARMED                          { quality = 1; price = 16000; };
	class     RHS_UH1Y_UNARMED_d                        { quality = 1; price = 16000; };
	class     RHS_Mi8mt_cargo_vdv                       { quality = 1; price = 8000; };
	class     RHS_Mi8amt_civilian                       { quality = 1; price = 8000; };
	class     rhs_ka60_c                                { quality = 1; price = 10000; };
	class 	  rhs_ka60_grey								{ quality = 1; price = 14000; };
	class     RHS_CH_47F                                { quality = 1; price = 50000; };
	class     RHS_CH_47F_light                          { quality = 1; price = 50000; };
	class     RHS_UH60M                                 { quality = 1; price = 50000; };
	class	  RHS_Mi8mt_vvs                             { quality = 1; price = 30000; };
	class	  RHS_Mi8amt_chdkz                          { quality = 1; price = 8000; };
	class     RHS_UH1Y                                  { quality = 1; price = 150000; };
	class     RHS_Mi24Vt_vvs                            { quality = 1; price = 200000; };
	class     RHS_C130J                                 { quality = 1; price = 40000; };
	class     GNT_C185F                                 { quality = 1; price = 5000; };
	class     rhsusf_CH53E_USMC                         { quality = 1; price = 32000; };