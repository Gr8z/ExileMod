...
ExilePlayerRadiation = 0;
{
	...
	if (_distance < (_x select 2)) exitWith
	{
		...
		{
			playSound [format ["GG_Sound_GeigerCounter_High0%1", 1 + (floor random 3)], true];
			_damage = 1/(1*60) * 2;
		}
		else 
		{
			...
			{
				playSound [format ["GG_Sound_GeigerCounter_Medium0%1", 1 + (floor random 3)], true];
				_damage = 1/(3*60) * 2;
			}
			else 
			{
				playSound [format ["GG_Sound_GeigerCounter_Low0%1", 1 + (floor random 3)], true];
				_damage = 1/(5*60) * 2;
			};
		};
