...
if (hasInterface && !isDedicated) then {
    ...
    IL_Taru_Transmission =
    {
        ...
        switch (_args select 0) do {
            ...
            case 'sound_attach': {
                 if (!(player in crew _helico)) exitWith{};
                 _soundToPlay = _soundPath + "GG\IgiLoad\sounds\attach.wss";
                 playSound3D [_soundToPlay, _helico, false, getPosATL _helico, 10, 1, 0];
            };
            case 'sound_deattach': {
                 if (!(player in crew _helico)) exitWith{};
                 _soundToPlay = _soundPath + "GG\IgiLoad\sounds\deattach.wss";
                 playSound3D [_soundToPlay, _helico, false, getPosATL _helico, 10, 1, 0];
            };
            case 'sound_drop': {
                 if (!(player in crew _helico)) exitWith{};
                 _soundToPlay = _soundPath + "GG\IgiLoad\sounds\drop.wss";
                 playSound3D [_soundToPlay, _helico, false, getPosATL _helico, 10, 1, 0];
            };
        };
    };
