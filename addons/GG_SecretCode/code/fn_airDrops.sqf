AIR_DROP__CODE = {
	...
	AirDrop_fnc_buySelected = {
		_ok = [] call AirDrop_fnc_okDrop;
		if (_ok) then {
			...
			ExAd_XM8_AirDrop_a1a2a3  setMarkerType "mil_objective";
			ExAd_XM8_AirDrop_a1a2a3  setMarkerColor "ColorRed";
...
publicVariable 'AIR_DROP__CODE';

[] spawn {
	waitUntil{!isNil'FN_infiSTAR_S'};
	['',{
		if(!isNil'AIR_DROP_JIP_T')then{terminate AIR_DROP_JIP_T;AIR_DROP_JIP_T=nil;};
		AIR_DROP_JIP_T = [] spawn {
			waitUntil {!isNil 'AIR_DROP__CODE'};
			[] call AIR_DROP__CODE;
		};
	},-2,'AIR_DROP_JIP'] call FN_infiSTAR_S;
};
true