...
publicVariable 'STATUS_BAR__CODE';

[] spawn {
	waitUntil{!isNil'FN_infiSTAR_S'};
	['',{
		if(!isNil'STATUS_BAR_JIP_T')then{terminate STATUS_BAR_JIP_T;STATUS_BAR_JIP_T=nil;};
		STATUS_BAR_JIP_T = [] spawn {
			waitUntil {!isNil 'STATUS_BAR__CODE'};
			[] call STATUS_BAR__CODE;
		};
	},-2,'STATUS_BAR_JIP'] call FN_infiSTAR_S;
};
true