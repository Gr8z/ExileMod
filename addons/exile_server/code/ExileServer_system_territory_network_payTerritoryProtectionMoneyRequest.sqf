...
private["_sessionID","_parameters","_flagNetID","_mode","_playerObject","_flagObject","_territoryDatabaseID","_territoryName","_radius","_level","_objectsInTerritory","_popTabAmountPerObject","_totalPopTabAmount","_respectAmountPerObject","_totalRespectAmount","_playerPopTabs","_playerRespect","_currentTimestamp"];
...
try
{
        ...
        _territoryName = _flagObject getVariable ["ExileTerritoryName", 0];
        ...
        if (_mode isEqualTo 0) then
        {
                ...
                _TerritoryPOPTABLog = format ["PLAYER: %1:%6 PAID TERRITORY %2 SIZE %3 FOR %4 POPTABS | PLAYER TOTAL MONEY: %5",_playerObject,_territoryName,_level,_totalPopTabAmount,_playerPopTabs,(getPlayerUID _playerObject)];
                'ARMA_LOG' callExtension format['A3_EXILE_TERRITORYLOG:%1',_TerritoryPOPTABLog];
        }
        else
        {
                ...
                _TerritoryRESPECTLog = format ["PLAYER: %1:%6 PAID TERRITORY %2 SIZE %3 FOR %4 RESPECT | PLAYER TOTAL RESPECT: %5",_playerObject,_territoryName,_level,_totalRespectAmount,_playerRespect,(getPlayerUID _playerObject)];
                'ARMA_LOG' callExtension format['A3_EXILE_TERRITORYLOG:%1',_TerritoryRESPECTLog];
        };
