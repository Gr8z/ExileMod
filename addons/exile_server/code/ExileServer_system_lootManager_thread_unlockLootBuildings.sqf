...
private["_lootLifetime","_building"];
_lootLifetime = ((getNumber(configFile >> "CfgSettings" >> "LootSettings" >>  "lifeTime") max 300) min 600);
{
	...
	{
		if !([getPosATL _building, getNumber(configFile >> "CfgSettings" >> "LootSettings" >>  "spawnRadius")] call ExileServer_util_position_isPlayerNearby) then
		{
