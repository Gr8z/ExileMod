...
_ZCP_RR_currentCapper setVariable['PLAYER_STATS_VAR', [_ZCP_RR_currentCapper getVariable ['ExilePurse', 0], _ZCP_RR_playerScore],true];
  ...
  if( _ZCP_RR_capperGroup != grpNull ) then {
    {
      if (_x != _ZCP_RR_currentCapper && _x distance2D _ZCP_RR_currentCapper < ZCP_CONFIG_GroupDistanceForRespect ) then {
        ...
        _x setVariable['PLAYER_STATS_VAR', [_x getVariable ['ExilePurse', 0], _ZCP_RR_newScore],true];
